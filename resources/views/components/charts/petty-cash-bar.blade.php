@props(['data', 'title' => 'Petty Cash Disbursements', 'subtitle' => 'Settled reimbursements by month'])
...
<div class="space-y-3">
    <div class="flex items-center justify-between">
        ...
        <span class="rounded-md bg-amber-50 px-2 py-0.5 font-mono text-[11px] font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
            Float Disbursements
        </span>
    </div>

    <svg viewBox="0 0 {{ $width }} {{ $height }}" class="block w-full overflow-visible select-none" style="max-height: 185px" role="img"
         aria-label="Monthly petty cash disbursements bar chart">
        <defs>
            <linearGradient id="pettyGradActive" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="#F59E0B" />
                <stop offset="100%" stop-color="#D97706" />
            </linearGradient>
            <linearGradient id="pettyGradDefault" x1="0" y1="0" x2="0" y2="1">
        ...
        @foreach ($data as $i => $point)
            ...
            <g class="group cursor-pointer">
                <rect x="{{ $x }}" y="{{ $y }}" width="{{ $barWidth }}" height="{{ $h }}" rx="6" ry="6"
                      ...
                      fill="{{ $isLatest ? 'url(#pettyGradActive)' : 'url(#pettyGradDefault)' }}">
                    <title>{{ $point['label'] }}: {{ \App\Support\Money::npr($point['amount']) }} ({{ $point['tokens'] }} tokens, {{ $point['paid'] }} paid)</title>
                </rect>

                @if ($isLatest)
                    <text x="{{ $x + $barWidth / 2 }}" y="{{ $y - 6 }}" text-anchor="middle"
                          class="fill-amber-600 font-mono text-[11px] font-bold dark:fill-amber-400 drop-shadow-sm">
                        {{ \App\Support\Money::format($point['amount']) }}
                    </text>
                @endif

                <text x="{{ $x + $barWidth / 2 }}" y="{{ $height - 8 }}" text-anchor="middle"
                      class="{{ $isLatest ? 'fill-amber-600 font-bold dark:fill-amber-400' : 'fill-slate-500 dark:fill-slate-400' }} font-mono text-[12px] group-hover:font-semibold">
                    {{ $point['label'] }}
                </text>
            </g>
        @endforeach
